...
class Kitchen(Screen):
    ...
    def show_plot(self, plot_type):
        ...
        if plot_type == "daily":
            url = f"{backend}/api/generate_plot_daily_sub_metering_1"
        elif plot_type == "weekly":
            url = f"{backend}/api/generate_plot_weekly_sub_metering_1"
        elif plot_type == "monthly":
            url = f"{backend}/api/generate_plot_monthly_sub_metering_1"
        else:
